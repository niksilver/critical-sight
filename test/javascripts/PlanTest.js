...
define(['Plan', 'Errors'], function(Plan, Errors) {
	describe("Plan", function() {
		...
		describe("periodList", function() {
			...
			it("should require every period to have an id", function() {
				...
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.BadlyDefinedObjectError("Found a period without an id"));
			});
			
			it("should require every period to have a start", function() {
				var spec = { periods: [{ id: 't0', notAStart: 1.0, duration: 1.0 }] };
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.BadlyDefinedObjectError("Found a period without a start"));
			});
		...
		describe("dependencyIDs", function() {
			...
			it("should give an error if some first dependency id is not a task ID", function() {
				...
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.UndefinedTaskError("Undefined period id 't3' among the dependencies"));
			});
			...
			it("should give an error if some second dependency id is not a task ID", function() {
				...
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.UndefinedTaskError("Undefined period id 't4' among the dependencies"));
			});
			...
			it("should give an error if some so-called pair is not an array", function() {
				...
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.BadlyDefinedObjectError("Expected a period id pair, but found 'Hello!'"));
			});
			...
			it("should give an error if some so-called pair doesn't have two elements (1)", function() {
				...
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.BadlyDefinedObjectError("Dependency 0 needs two elements, found 3"));
			});
			...
			it("should give an error if some so-called pair doesn't have two elements (2 - to avoid faking)", function() {
				...
				expect( function(){ new Plan(spec); } ).toThrow(
						new Errors.BadlyDefinedObjectError("Dependency 2 needs two elements, found 1"));
			});
		});
